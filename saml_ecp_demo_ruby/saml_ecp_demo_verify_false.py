...
def get_xml_element(context_node, required, xpath_expr, description=None):
...
def get_xml_element_text(context_node, required, xpath_expr, description=None):
...
class ECPFlow:
    ...
    def __init__(self, sp_resource, idp_endpoint, user, password, idp_auth_method, log_categories):
    ...
    def pretty_print_paos_request_info(self, log_categories, msg=None):
    ...
    def process_paos_request(self):
        ...
        self.sp_response_consumer_url = get_xml_element_text(self.paos_request_xml, True,  '/soap:Envelope/soap:Header/paos:Request/@responseConsumerURL')
        self.sp_message_id            = get_xml_element_text(self.paos_request_xml, False, '/soap:Envelope/soap:Header/paos:Request/@messageID')
        self.provider_name            = get_xml_element_text(self.paos_request_xml, False, '/soap:Envelope/soap:Header/ecp:Request/@ProviderName')
        self.sp_is_passive            = get_xml_element_text(self.paos_request_xml, False, '/soap:Envelope/soap:Header/ecp:Request/@IsPassive')
        self.sp_issuer                = get_xml_element_text(self.paos_request_xml, True,  '/soap:Envelope/soap:Header/ecp:Request/saml:Issuer')
    ...
    def send_sp_response(self):
        ...
        print("\nJJV A02 : %s sp_response_text : \n%s" % (inspect.stack()[0].function, sp_response_text), flush=True)


        headers = { 'Content-Type': 'application/vnd.paos+xml', }

        print("\nJJV B01 : %s SP Endpoint: %s\n" % (inspect.stack()[0].function, self.sp_response_consumer_url), flush=True)
